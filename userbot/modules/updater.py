    ...
    if HEROKU_API_KEY is not None:
        ...
        if heroku_app is None:
            await edit_or_reply(
                xx,
                f"{txt}\n"
                "**Kredensial Heroku tidak valid untuk deploy Js-Userbot dyno.**",
            )
            return repo.__del__()
        try:
        ...
        await edit_or_reply(
            xx, "`Js-Userbot Berhasil Di upgrade! Userbot bisa di gunakan kembali.`"
        )
...
async def update(xx, repo, ups_rem, ac_br):
    try:
        ...
        repo.git.reset("--hard", "FETCH_HEAD")
    await edit_or_reply(
        xx, "`Js-Userbot Berhasil Diupdate! Userbot bisa di Gunakan Lagi.`"
    )
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}update`\
        \n  •  **Function : **Untuk Melihat Pembaruan Terbaru Js-Userbot.\
        \n\n  •  **Syntax :** `{cmd}update deploy`\
        \n  •  **Function : **Untuk MengUpdate Fitur Terbaru Dari Js-Userbot.\
    "
    }
)
